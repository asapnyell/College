...
- **Conceitos de POO usados:** Herança (`Pessoa` → `Professor`/`Aluno`), encapsulamento (atributo privado `__notas` e acesso via `@property media`), tratamento de exceções (`NotaInvalidaError`), sobrescrita de métodos especiais (`__str__`) e agregação (uma `Turma` contém vários `Aluno`).
- **Como o diagrama ajudou:** O diagrama clarificou responsabilidades e relações entre classes, deixando explícito quem gerencia dados (ex.: `Turma` agrega `Aluno`) e onde ficam regras de negócio (validação de notas em `Aluno`).
